...
//   ({ children, className, variant, width, asChild = false, ...props }, ref) => {
...
const Container = React.forwardRef<HTMLDivElement, ContainerProps>(
  ({ children, className, variant, width, animate = true, asChild = false, ...props }, ref) => {
    const Comp = asChild ? Slot : "div";
    const internalRef = useRef(null);
    const isInView = useInView(internalRef, { once: true });
    ...
    return (
      <Comp
        className={cn(containerVariants({ variant, width, animate, className }))}
        ref={internalRef}
        {...props}
      >
        {content}
